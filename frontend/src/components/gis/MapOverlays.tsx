...
export default function MapOverlays({ cursor, year }: { cursor: CursorState; year: number }) {
  ...
  return (
    <>
      ...
      <div className="pointer-events-none absolute left-4 top-4 z-[500] grid h-9 w-9 place-items-center rounded-full border border-border bg-[var(--surface-0)]/90 shadow-md backdrop-blur">
        <div className="relative flex h-full w-full items-center justify-center">
          <Compass className="h-4.5 w-4.5 text-primary" />
          <span className="absolute top-0.5 font-mono text-[8px] font-bold text-primary">N</span>
        </div>
      </div>

      {/* Compact NDVI Values Scale Box */}
      <div className="pointer-events-none absolute bottom-4 left-4 z-[500] w-48 rounded-lg border border-border bg-[var(--surface-0)]/90 p-2 shadow-md backdrop-blur">
        <div className="mb-1 flex items-center justify-between text-[9px] font-bold uppercase tracking-wider text-muted-foreground font-mono">
          <span className="text-foreground">NDVI Scale</span>
          <span className="font-bold text-primary">−0.2 → 0.95</span>
        </div>

        <div className="h-1.5 w-full rounded-full ndvi-swatch border border-border/60 shadow-inner" />

        <div className="mt-1 flex justify-between font-mono text-[8px] font-bold text-foreground/80">
          <span>Water</span>
          <span>Bare</span>
          <span>Sparse</span>
          <span>Mod</span>
